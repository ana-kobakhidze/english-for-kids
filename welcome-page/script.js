...
const concatTitle = (title) => {
  ...
  const imageTitle =
    titleStr.substring(0, 1).toLowerCase() + titleStr.substring(1);
  return imageTitle;
};
// first page
if (
...
// storing words in local storage
  let cardObject = [];
  categoryCardData.forEach((card) => {
    for (let key in card) {
      cardObject.push(card[key].word);
    }
  });
  localStorage.setItem('words', JSON.stringify(cardObject));
